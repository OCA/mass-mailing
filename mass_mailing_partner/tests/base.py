...
class BaseCase(TransactionCase):
    def setUp(self):
        ...
        self.category_0 = self.env["res.partner.category"].create(
            {"name": "Test Category 0"}
        )
        self.category_2 = self.env["res.partner.category"].create(
        ...
        self.category_3 = self.env["res.partner.category"].create(
    ...
    def check_mailing_contact_partner(self, mailing_contact):
        if mailing_contact.partner_id:
            ...
            expected_company_name = (
                mailing_contact.partner_id.parent_id.name
                or mailing_contact.partner_id.company_name
                or mailing_contact.partner_id.commercial_company_name
                or False
            )
            self.assertEqual(expected_company_name, mailing_contact.company_name)
            self.assertEqual(
